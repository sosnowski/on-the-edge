...
<button
	...
	class="fixed rounded-full bg-red-500 w-10 h-10 text-white p-2 bottom-2 right-2"
	><svg
...
{#if formVisible}
	<div transition:fade class="pointer-events-none fixed left-0 right-1 bottom-8 px-6 pb-6">
		<div
			class="pointer-events-auto ml-auto max-w-lg rounded-md bg-white p-4 shadow-lg border border-slate-100"
		>
			<slot />
		</div>
	</div>
